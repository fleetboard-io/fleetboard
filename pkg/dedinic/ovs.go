...
func configureContainerNic(nicName, ifName, ipAddr, gateway string, isDefaultRoute, detectIPConflict bool,
	routes []request.Route, macAddr net.HardwareAddr, netns ns.NetNS, mtu int, nicType string, gwCheckMode int,
	u2oInterconnectionIP string) error {
	var err error
...
func configureNic(link, ip string, macAddr net.HardwareAddr, mtu int, detectIPConflict bool) error {
	...
	for ip, address := range ipAddMap {
		...
		if detectIPConflict && addr.IP.To4() != nil {
			...
			mac, err = util.ArpDetectIPConflict(link, ipStr, macAddr)
			if err != nil {
				err = fmt.Errorf("failed to detect address conflict for %s on link %s: %v", ipStr, link, err)
				klog.Error(err)
				return err
			}
			if mac != nil {
				return fmt.Errorf("IP address %s has already been used by host with MAC %s", ipStr, mac)
			}
		}
...
func waitNetworkReady(nic, ipAddr, gateway string, underlayGateway, verbose bool, maxRetry int) error {
	...
	for i, gw := range strings.Split(gateway, ",") {
		...
		if underlayGateway && util.CheckProtocol(gw) == ProtocolIPv4 {
			...
			if err != nil {
				err = fmt.Errorf("network %s with gateway %s is not ready for interface %s after %d checks: %v",
					ips[i], gw, nic, count, err)
				klog.Warning(err)
				return err
			}
			if verbose {
				klog.Infof("MAC addresses of gateway %s is %s", gw, mac.String())
				klog.Infof("network %s with gateway %s is ready for interface %s after %d checks",
					ips[i], gw, nic, count)
			}
		} else {
...
func setEncapIP(ip string) error {
	...
	encapIP := fmt.Sprintf("external-ids:ovn-encap-ip=%s", ip)
	raw, err := exec.Command(
		"ovs-vsctl", "set", "open", ".", encapIP).CombinedOutput()
